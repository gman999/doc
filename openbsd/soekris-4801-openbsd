...
The venerable Soekris 4801 with its 128 megabyte of RAM and slow 233 MHz processor can still be of utility.
...
Plug in the CF card adapter and note the device listed to standard output, normally sd with a respective number, such as sd2.
...
Next the list of interfaces will be listed, but wired and the recognized wireless devices. This can be configured for DHCP, as it will only be used to pull the relevant install sets from an OpenBSD mirror. Note that the interfaces only apply to the bootstrap computer, and will need to be adjusted manually on the actual Soekris device.
...
for the functions listed previously, X Windows and xmd(1) are unnecessary.
...
In this case, standard output noted that the CF card is located on sd2. An incorrect choice here, such as choosing the bootstrap computer's disk, will mean overwriting the wrong disk.
